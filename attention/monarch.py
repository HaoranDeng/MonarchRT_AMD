...
def _monarch_one_step_factors(q, k, scale, q_init, random_seed=None):
    ...
    Standard MRT-1 corresponds to q_init="ith", where the initial L is identity
    and a_R[..., k, j] is Q[..., k, j].
    """
...
def _monarch_one_step_attention_blocks(q, k, v, scale, query_outer_chunk, q_init, random_seed):
...
def monarch_attn(
    ...
    if num_iters == 1 and _is_identity_q_init(q_init):
        ...
        if causal_block_size:
            raise ValueError("non-identity Monarch q_init is not supported with causal block masks.")
        z = _monarch_one_step_attention_blocks(
            q, k, v, sm_scale, query_outer_chunk, q_init, random_seed
        )
    else:
